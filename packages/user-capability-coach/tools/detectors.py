...
CONFLICT_MARKER_RE = re.compile(
    ...
    r"|\b(shorter|shortest|concise|brief)\b.{0,40}\b(detailed|comprehensive|thorough|all\s+\w+)\b"
    r"|\b(no dependenc|without .*library|no external)\b.{0,60}\b(use|import|with)\s+\w+"
    r"|\b(faster|cheaper|simpler)\b.{0,40}\b(and|but|while)\b.{0,20}\b(more|still|still\s+keep)\b"
    r"|要快.{0,15}又要|既.{0,10}(还|又).{0,10})",
    re.IGNORECASE,
)
