...
今天的 AI 助手都是**答案友好**的：你问得不清楚，它照样努力猜、努力答。单次任务看起来没问题，但拉长时间线就会发现一个模式：

- 当下任务完成了
- 用户下次还是那么问
- 澄清轮次、返工、风格错配反复发生
- **用户的提问能力并没有增长，只是学会了容忍助手的猜测**

几乎所有现有 AI 工具都有同一个缺口：**模型在「回答」上越来越强，用户在「提问」上却停在原地。** `user-capability-coach` 下的注是：助手应该同时悄悄地帮用户成长，但**永远不打断当前任务**。

这个定位故意远离两个反模式：

1. **提示警察（prompt police）** —— 每句话都被批改。用户几天就会关掉。
2. **纯静默修正** —— 助手内部改写你的意思继续执行，但你永远学不到哪里可以做得更好。

目标用户**不是新手**（新手需要的是更多答案，不是辅导），而是**重度使用 AI 助手的人** —— 他们的效率瓶颈不再是"模型够不够聪明"，而是"我能不能用更少轮次、更清楚地表达我要什么"。coach 就是把这种杠杆以极短、可忽略、绝不评判的方式点出来，让用户 1 秒内决定采不采纳。

**成功标准不是"提醒了多少次"**，而是 plan 里明确定下的三个结果：首轮命中率提高、重复性提问缺口减少、用户仍然感觉**被帮助**而不是**被观察**。
